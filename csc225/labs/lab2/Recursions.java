/* Recursions.java
   ...
   Rahnuma Islam Nishat and Michael Simpson - 01/13/2018
*/
...
public class Recursions{
        ...
        */
		...
		if (head.next!=null) return head.value+Sum(head.next);
		return head.value;
        }
        /* FindMax(head)
        ...
        */
		...
		if (head.next!=null) return Math.max(head.value, FindMax(head.next));
		//the way the above line works is by doing no comparison until it hits end of array
		//and then it compares the last two, and returns the bigger
        ...
        */
	//CREATE a NEW LIST.
	//The old lists are left untouched!
        public ListNode JoinLists(ListNode h1, ListNode h2){
		if (h1==null) return h2;
		return new ListNode(h1.value, JoinLists(h1.next, h2));
		//listnode(value, next)
		//base case: hitting the null node of the first list
		//So, the new list kinda forks into the second old list. Points, eventually, to the beginning of the second old list. Everything before that is cloned from the first old list.
        }
        ...
        */
		...
		if (head.next==null) return new ListNode(head.value+index);
		return new ListNode(head.value+index, ValuePlusIndex(index+1, head.next));
        }
        ...
        */
		...
		if (head.value%2==0){
			return new ListNode(head.value, ExtractEven(head.next));
		}
